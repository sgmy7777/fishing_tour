...
if (phoneInput) {
  // При фокусе ставим + если поле пустое
  ...
  phoneInput.addEventListener('keydown', (e) => {
    const servicKeys = ['Backspace', 'Delete', 'ArrowLeft', 'ArrowRight', 'Tab', 'Home', 'End'];
  ...
  // Защита от вставки — оставляем только цифры после +
  ...
  // Гарантируем формат: всегда начинается с + и только цифры после
  phoneInput.addEventListener('input', () => {
    let val = phoneInput.value;
    const digits = val.replace(/[^\d]/g, '');
    phoneInput.value = '+' + digits;
    // Если стёрли +, не даём полю остаться пустым без него
...
function validatePhone(value) {
  ...
  if (!/^\+\d{7,15}$/.test(v)) return 'Формат: + и от 7 до 15 цифр, например +79991234567';
  return null;
}
...
function validateEmail(value) {
  ...
  // Строгая проверка: local-part@domain.tld (tld минимум 2 символа)
...
if (bookingFormElement && formNoteElement) {
  ...
  FIELDS.forEach(([fieldId, fieldName, validator]) => {
    ...
    // Снимаем ошибку при редактировании
  ...
  bookingFormElement.addEventListener('submit', (e) => {
    ...
    // Здесь подключите EmailJS или другой сервис отправки.
